    ...
    public static class AssemblyInfo
    {
        public static string DefaultLangVersion
        {
            get
            {
                ...
                p.StartInfo.FileName               = $@"{Environment.GetEnvironmentVariable("VSAPPIDDIR")}..\..\MSBuild\Current\Bin\Roslyn\csc.exe";
                p.StartInfo.Arguments              = "-langversion:?";
        ...
        private static string GetExecutingAssemblyAttribute<T>(this Assembly asm, Func<T, string> value)
            where T : Attribute
        {
            var attribute = Attribute.GetCustomAttribute(asm, typeof(T)) as T;
            return attribute != null ? value.Invoke(attribute) : null;
        }
    }
}